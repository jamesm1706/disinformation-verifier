...
def test_build_dataframe_from_list_of_dicts():
    """A list of dicts becomes a DataFrame with matching rows and columns."""
    records = [
        {"a": 1, "b": "x"},
        {"a": 2, "b": "y"},
    ]
...
def test_build_dataframe_from_empty_list():
    """An empty list produces an empty DataFrame."""
    df = build_dataframe([])
...
def test_clean_bool_value_converts_non_bool_to_false():
    """A non-bool value becomes False."""
...
def test_transform_produces_clean_dataframe():
    """transform() cleans every column of a raw record end to end."""
    records = [
        {
